...
# Add churn project to path for config access
sys.path.append('/home/magicdash/astro-airflow/churn')
...
PROJECT_ROOT = '/home/magicdash/astro-airflow/churn'
...
# DBT Profile Configuration with environment variable support
profile_config = ProfileConfig(
    ...
    profile_mapping=SnowflakeUserPasswordProfileMapping(
        ...
        profile_args={
            ...
            "user": "MAGICDASH",
            "password": os.getenv('SNOWFLAKE_PASSWORD', 'Permataputihg101'),  # Use env var if available
            "role": "ACCOUNTADMIN", 
            "database": "DATABASE",
        ...
        with snowflake.connector.connect(**conn_params) as conn:
            with conn.cursor() as cursor:
                ...
                SELECT 
                    ...
                    MAX(TRY_TO_TIMESTAMP(TENURE)) as latest_update
                ...
                if latest_update:
                    logging.info(f"Latest data update: {latest_update}")
...
def check_dbt_environment(**context):
    ...
    try:
        dbt_executable = '/home/magicdash/astro-airflow/venv/bin/dbt'
        profiles_dir = '~/.dbt'
        
        # Check if dbt executable exists
        if not os.path.exists(dbt_executable):
            logging.warning(f"DBT executable not found at {dbt_executable}")
            return False
        
        # Check if profiles directory exists
        expanded_profiles_dir = os.path.expanduser(profiles_dir)
        if not os.path.exists(expanded_profiles_dir):
            logging.warning(f"DBT profiles directory not found at {expanded_profiles_dir}")
            return False
        
        logging.info("DBT environment check passed")
        return True
...
def upload_to_s3(**context):
    ...
    try:
        ...
        run_date = context['ds_nodash']  # YYYYMMDD
        execution_date = context['execution_date']
        run_id = context['run_id']
        
        logging.info(f"🚀 Starting S3 upload for run {run_id}")
        ...
        for file_path in pred_files:
            try:
                ...
                upload_summary['predictions_uploaded'] += 1
            except Exception as e:
        ...
        for file_path in processed_files:
            try:
                ...
                s3_client.upload_file(file_path, S3_CONFIG['buckets']['churn_results'], s3_key)
                logging.info(f"📋 Uploaded: s3://{S3_CONFIG['buckets']['churn_results']}/{s3_key}")
                upload_summary['data_uploaded'] += 1
            except Exception as e:
                logging.error(f"Failed to upload {filename}: {str(e)}")
                upload_summary['failed_uploads'] += 1
            upload_summary['total_files'] += 1
        ...
        for file_path in model_files:
            try:
                filename = os.path.basename(file_path)
                s3_key = f'models/{run_date}/{run_id}/{filename}'
                ...
                upload_summary['models_uploaded'] += 1
            except Exception as e:
                logging.error(f"Failed to upload {filename}: {str(e)}")
                upload_summary['failed_uploads'] += 1
            upload_summary['total_files'] += 1
        ...
        # Upload run summary as JSON
        summary_key = f'run_summaries/{run_date}/{run_id}_summary.json'
        s3_client.put_object(
            Bucket=S3_CONFIG['buckets']['churn_results'],
            Key=summary_key,
            Body=json.dumps(run_summary, indent=2),
            ContentType='application/json'
        )
        
        success_rate = (upload_summary['total_files'] - upload_summary['failed_uploads']) / upload_summary['total_files'] * 100
        
        logging.info("✅ S3 Upload Summary:")
        logging.info(f"   Predictions: {upload_summary['predictions_uploaded']} files")
        logging.info(f"   Models: {upload_summary['models_uploaded']} files")
        logging.info(f"   Data: {upload_summary['data_uploaded']} files")
        logging.info(f"   Success rate: {success_rate:.1f}%")
        logging.info(f"   Summary: s3://{S3_CONFIG['buckets']['churn_results']}/{summary_key}")
...
with TaskGroup('dbt_transformations', dag=dag) as dbt_group:
    ...
    dbt_run_task = BashOperator(
        ...
        bash_command=f'''
        cd {DBT_PROJECT_DIR} && 
        /home/magicdash/astro-airflow/venv/bin/dbt run --profiles-dir ~/.dbt || 
    ...
    dbt_test_task = BashOperator(
        ...
        bash_command=f'''
        cd {DBT_PROJECT_DIR} && 
        /home/magicdash/astro-airflow/venv/bin/dbt test --profiles-dir ~/.dbt || 
