...
logger.info(f"Churn path detected: {churn_path}")

# Verify monitoring module is accessible
if churn_path:
    monitoring_path = os.path.join(churn_path, 'monitoring')
    if os.path.exists(monitoring_path):
        logger.info(f"Monitoring directory found at: {monitoring_path}")
        init_file = os.path.join(monitoring_path, '__init__.py')
        if os.path.exists(init_file):
            logger.info("Monitoring __init__.py file exists")
        else:
            logger.warning("Monitoring __init__.py file missing")
    else:
        logger.warning(f"Monitoring directory not found at: {monitoring_path}")
...
def start_metrics_server(**context):
    ...
    try:
        ...
        logger.info(f"Current sys.path: {sys.path[:3]}...")  # Show first 3 entries
        
        # Try to import monitoring module with detailed error info
        try:
            import monitoring
            logger.info(f"Monitoring module imported from: {monitoring.__file__}")
        except ImportError as import_error:
            logger.error(f"Failed to import monitoring module: {import_error}")
            logger.info("Available directories in churn path:")
            if churn_path and os.path.exists(churn_path):
                for item in os.listdir(churn_path):
                    item_path = os.path.join(churn_path, item)
                    if os.path.isdir(item_path):
                        logger.info(f"  - {item}")
            return "Metrics server not available - monitoring module import failed"
        
        try:
            from monitoring.metrics_collector import ChurnPipelineMetrics
            logger.info("Successfully imported ChurnPipelineMetrics")
        except ImportError as e:
            logger.error(f"Failed to import ChurnPipelineMetrics: {e}")
            return f"Metrics server failed - ChurnPipelineMetrics import error: {e}"
        ...
        # Return error message instead of raising to avoid task failure
        return f"Metrics server failed: {str(e)}"
...
def start_stream_processor(**context):
    ...
    try:
        ...
        try:
            ...
            logger.warning("RealTimeStreamProcessor not available - streaming functionality limited")
            return "Stream processor not available - module not found"
        
        # Try to initialize the processor, which may fail if Kafka is not available
        try:
            processor = RealTimeStreamProcessor()
        except Exception as kafka_error:
            # Check if it's a Kafka connection error
            if "NoBrokersAvailable" in str(kafka_error) or "Connection refused" in str(kafka_error):
                logger.warning(f"Kafka not available for stream processor: {kafka_error}")
                return f"Stream processor not started - Kafka unavailable: {kafka_error}"
            else:
        ...
        # Return error message instead of raising to avoid task failure
        return f"Stream processor failed: {str(e)}"
    ...
    try:
        ...
        def check_kafka_availability():
            try:
                ...
                sock.settimeout(3)
                result = sock.connect_ex(('churn-kafka', 9092))
                sock.close()
                return result == 0
            except:
                return False
        ...
        if not check_kafka_availability():
            logging.warning("Kafka is not available at churn-kafka:9092")
